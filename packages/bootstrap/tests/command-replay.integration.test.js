...
describe('command replay full-stack integration', { timeout: 30000, sequential: true, shuffle: false }, () => {
  let container;
  ...
  beforeAll(() => {
    ...
    return new MongoDBContainer('mongo:7')
      ...
      .then((server) => {
        ...
        console.log(`[SETUP] Command processor started on port ${commandPort}`);
  ...
  afterAll(() =>
    Promise.resolve()
      ...
      .then(() =>
        cleanupClient ? cleanupClient.close() : undefined,
      )
      .then(() =>
        container ? container.stop() : undefined,
      )
      .then(() =>
        unlink(commandRecordFile).catch(() => {}),
      ),
  );
  ...
  const clearReadModel = () =>
    cleanupClient.db('readmodel-test').listCollections().toArray()
      .then((collections) =>
        collections.reduce(
          (chain, col) => chain.then(() => cleanupClient.db('readmodel-test').collection(col.name).deleteMany({})),
          Promise.resolve(),
        ),
      );
  ...
  test('complete command replay cycle without auth', () => {
    ...
    return sendCommand('item-1', 'Item One')
      ...
      .then((res3) => {
        ...
        console.log('[TEST no-auth] All 3 commands sent, waiting for projections');
      ...
      .then((recordedContent) => {
        ...
        console.log('[TEST no-auth] Clearing event store, read model, aggregate store');
      ...
      .then(() => {
        ...
        return sendCommand('auth-item-1', 'Auth Item One', authHeaders)
          ...
          .then((recordedContent) => {
            ...
            return clearEventStore()
              ...
              .then((clearedItems) => {
                ...
                return readFile(commandRecordFile, 'utf8').then((preReplayContent) => {
          ...
          .then((preReplayLineCount) => {
            ...
            console.log('[TEST auth] Verifying replayed commands recorded auth info');
            return readFile(commandRecordFile, 'utf8').then((postReplayContent) => {
