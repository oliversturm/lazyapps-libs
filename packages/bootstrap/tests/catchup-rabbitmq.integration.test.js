  ...
  beforeAll(() => {
    ...
    return Promise.all([
      ...
      .then((context) => {
        rmContext = context;
        console.log(
          `[ENV rmq] RM context initialized, lifecycle: ${!!context.lifecycleManager}`,
        );
        ...
        waitForCondition(
          () =>
            cleanupClient
              ...
              .toArray()
              .then((items) =>
                items.length === 5 ? true : `count=${items.length}`,
              ),
          5000,
          100,
          'rmq items projected (5)',
        ),
      )
      .then(() =>
        cleanupClient
