          ...
          waitForCondition(
            () =>
              fetchRM('/admin/readmodel').then(({ body }) => {
                ...
                if (
                  items &&
                  (items.state === 'catchup' || items.state === 'live')
                )
                  return true;
                return `state=${items?.state || 'not found'}`;
              }),
